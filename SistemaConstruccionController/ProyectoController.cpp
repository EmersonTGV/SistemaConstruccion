...
List<Proyecto^>^ ProyectoController::buscarProyectos(String^ departamento) {
	...
	for each (String ^ lineaProyecto in lineas) {
		...
		if (departamentoProyecto->Contains(departamento)) {
			Proyecto^ objProyecto = gcnew Proyecto(codigoProyecto, fechaInicioProyecto, fechaEntregaProyecto, departamentoProyecto, provinciaProyecto, distritoProyecto, nroEdificiosProyecto,nombreProyecto);
